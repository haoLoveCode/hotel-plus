<template>
  <el-dialog title="用户编辑" :center="true" :visible.sync="editVisible">
    <div class="add-body">
      <el-form
          ref="submitForm"
          :model="submitData"
      ...
      >
        ...
        <div class="add-body-div">
          ...
          <div class="add-item-view" v-if="false">
...
export default {
  ...
  methods: {
    ...
    async setAvatarUrl(data) {
      ...
      this.submitData.avatarUrlList = this.$isNull(this.submitData.avatarUrlList) ?
          new Array() : this.submitData.avatarUrlList
      ...
      this.submitData.avatarUrlList.push(item)
    },
    async setOtherData(data) {
        await this.setAvatarUrl(data);
    },
    //处理初始化
    async init(data) {
    ...
    handleSubmit() {
      this.$refs.submitForm.validate((valid) => {
        ...
        try {
          Api.updateAppAuthUser(params).then((res) => {
            if (res.success) {
              this.editVisible = false;
              ...
              let data = res.data;
              console.log('编辑结果:' + JSON.stringify(data));
              this.clearAll()
              localStorage.clear();
