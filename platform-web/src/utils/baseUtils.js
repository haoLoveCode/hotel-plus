...
export const filePrefix = 'https://skywalking-web.oss-cn-chengdu.aliyuncs.com/' //图片拼接前缀
const baseUtils = {
  setLocalStorgae(key,value) {
  ...
  //验证Token
  async verifyToken(){
    let result = await Api.verifyToken({
      ...
      if (res.success) {
        ...
        let verifyStatus = data.verifyStatus;
        if(!verifyStatus){
          Message.error({
            message:`需要登录`,
            duration:2000
