package cn.common.repository.repository.biz;

import cn.common.repository.entity.biz.BasketData;
import org.apache.ibatis.annotations.Mapper;
import com.github.yulichang.base.MPJBaseMapper;

/**
 * @author Singer
 * @packageName cn.singer.repository.entity
 * @Description: 购物车信息->Repository
 * @date 2024-03-06
 */
@Mapper
public interface BasketDataRepository extends MPJBaseMapper<BasketData> {


    /**
     * 根据业务主键批量删除购物车信息
     * @title: BasketDataRepository.java
     * @author Singer
     * @date 2024-03-06 11:05
     * @param mainIdList 业务主键ID集合
     * @return List

    @Delete(
        "<script>" +
        " DELETE FROM basket_data WHERE 1=1 AND  " +
        " basket_data_id IN " +
        " <foreach collection='mainIdList' item= 'item' index= 'index' open='(' separator=',' close=')' >" +
        "   #{item} " +
        " </foreach>" +
        "</script>"
        )
    void batchDeleteItem(@Param("mainIdList") List<String> mainIdList);*/

}
