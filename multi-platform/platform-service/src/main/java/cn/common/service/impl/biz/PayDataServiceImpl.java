...
import cn.common.resp.biz.openBiz.PayDataExportResp;
...
public class PayDataServiceImpl implements PayDataService {
     ...
     */
    @Override
    public void exportData(PayDataReq pageReq){
        log.info(">>>>>>>>>>>>>>>>>数据导出Req {} <<<<<<<<<<<<<<<<", JSON.toJSONString(pageReq));
        //构建查询条件
        LambdaQueryWrapper<PayData> pageWrapper = new LambdaQueryWrapper<>();
        setPageCriteria(pageWrapper,pageReq);
        pageWrapper.orderBy(true,false,PayData::getCreateTime);
        List<PayData> pageList = payDataRepository.selectList(pageWrapper);
        if (CollectionUtils.isEmpty(pageList)) {
            return;
        }
        List<PayDataExportResp> respList =
            mapperFacade.mapAsList(pageList, PayDataExportResp.class);
        try {
            ...
            try {
                // 这里 需要指定写用哪个class去写
                excelWriter = EasyExcel.write(response.getOutputStream(), PayDataExportResp.class).build();
    ...
    public void addItem(PayDataAddReq addReq){
        log.info(">>>>>>>>>>>>>>>>>新增Req {} <<<<<<<<<<<<<<<<", JSON.toJSONString(addReq));
        PayData entity = mapperFacade.map(addReq, PayData.class);
        try {
    ...
    public void batchDeleteItem(BaseDeleteReq req){
        ...
        List<PayData> entityList = payDataRepository.selectList(
            new LambdaQueryWrapper<PayData>().in(PayData::getPayDataId,mainIdList));
        entityList.stream().forEach(item -> {
            payDataRepository.deleteById(item);
        });
    }
     ...
     */
    @Override
    public List<PayDataResp> queryAllPayData(){
        List<PayData> entityList = payDataRepository.selectList(new LambdaQueryWrapper<>());
            if(CollectionUtils.isEmpty(entityList)){
            return Lists.newArrayList();
        }
        return mapperFacade.mapAsList(entityList,PayDataResp.class);
    }
     ...
     */
    @Override
    public Pagination<PayDataResp> queryByPage(
        PayDataReq pageReq){
        ...
        List<PayDataResp> respList =
            mapperFacade.mapAsList(pageList, PayDataResp.class);
     ...
     * @return
     */
    private void setPageCriteria(LambdaQueryWrapper<PayData> pageWrapper, PayDataReq pageReq){

        if(!CheckParam.isNull(pageReq.getOutTradeNo())){
            pageWrapper.like(PayData::getOutTradeNo,pageReq.getOutTradeNo());
        }

        if(!CheckParam.isNull(pageReq.getPayData())){
            pageWrapper.like(PayData::getPayData,pageReq.getPayData());
        }
    }
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED,rollbackFor = Exception.class)
    public void updateItem(PayDataUpdateReq updateReq){
        log.info(">>>>>>>>>>>>>>>>>更新请求参数 {} <<<<<<<<<<<<<<<<", JSON.toJSONString(updateReq));
        String mainId = updateReq.getPayDataId();
        PayData result = payDataRepository.selectOne(new LambdaQueryWrapper<PayData>()
                    .eq(PayData::getPayDataId,mainId));
        if (CheckParam.isNull(result)) {
            return;
        }
        setNeedUpdateItem(result,updateReq);
        payDataRepository.updateById(result);
    }
     ...
     * @param entity 产业
     */
    private void setNeedUpdateItem(PayData entity,
        PayDataUpdateReq updateReq){
        if(!CheckParam.isNull(updateReq.getOutTradeNo())){
            entity.setOutTradeNo(updateReq.getOutTradeNo());
        }
        if(!CheckParam.isNull(updateReq.getPayData())){
            entity.setPayData(updateReq.getPayData());
        }
    }
}
