...
import cn.common.resp.biz.platform.SalesItemExportResp;
...
public class SalesItemServiceImpl implements SalesItemService {
     ...
     */
    @Override
    public void exportData(SalesItemReq pageReq){
        log.info(">>>>>>>>>>>>>>>>>商品信息数据导出Req {} <<<<<<<<<<<<<<<<", JSON.toJSONString(pageReq));
        //构建查询条件
        ...
        List<SalesItemExportResp> respList =
            mapperFacade.mapAsList(pageList, SalesItemExportResp.class);
        try {
            ...
            try {
                // 这里 需要指定写用哪个class去写
                excelWriter = EasyExcel.write(response.getOutputStream(), SalesItemExportResp.class).build();
