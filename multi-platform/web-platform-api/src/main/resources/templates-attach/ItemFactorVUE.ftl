    ...
    <div id="advanced-search-div">
        <a-form-model
            ...
            <a-row :gutter="5">
                ...
                <a-col
                        :span="12"
                        :style="{ display: expandStatus ? 'block' : 'none' }">
                    <a-form-model-item
                            has-feedback
                            ref="${column.camelCaseName}"
                            label="${column.columnNameDesc}"
                            prop="${column.camelCaseName}">
                        <a-input
                                placeholder="请输入${column.columnNameDesc}"
                                v-model="queryParams.${column.camelCaseName}"
                                @blur="
                         () => {
                           $refs.${column.camelCaseName}.onFieldBlur();
                         }"
                        />
                    </a-form-model-item>
                </a-col>
            ...
            <a-row>
                <a-col :span="24" :style="{ textAlign: 'right' }">
                    ...
                    <a :style="{ marginLeft: '8px', fontSize: '12px' }" @click="toggleExpand">
                        展开 <a-icon :type="expandStatus ? 'up' : 'down'" />
                    </a>
                </a-col>
            </a-row>
        </a-form-model>
...
export default {
    ...
    data () {
      return {
        locale:locale,
        dataBeginAndEndTimeFormat:this.$bizConstants.dataBeginAndEndTimeFormat,
        ...
        validateRules: {
            <#list columnList as column>
            ${column.camelCaseName}: [
                { required: false, message: '请规范填写${column.columnNameDesc}', trigger: 'blur' }
            ],
            </#list>
            dataType: [
                {
