            ...
            <el-form :inline="true" ref="pageForm">
                <div class="search-param-view">
                        ...
                        <div class="search-item-view">
                            <el-form-item label="${column.columnNameDesc}">
                                <el-input
                                    ...
                                    placeholder="${column.columnNameDesc}"
                                ></el-input>
                            </el-form-item>
                        </div>
                        </#list>
                    ...
                    <div class="search-view">
                        ...
                        <div class="search-item-view" v-if="isAuth('${camelCaseEntityName}:import')">
                            <el-form-item>
                                <!--导入-->
                                <UploadExcel
                                    :headers="uploadHeaders" :uploadApi="importExcelUrl"
                                    :fileList="excelList" :buttonText="buttonText" :upLoadParams="{}"
                                    ref="excelUploader" @onSuccess="onExcelSuccess"
                                    @handleRemove="handleExcelRemove">
                                </UploadExcel>
                            </el-form-item>
                        </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                buttonText: `导入信息`,
                excelList: [],
                importExcelUrl: process.env.BASE_API + `/api/v1/${camelCaseEntityName}/importData`,
                uploadHeaders: {
        ...
        methods: {
            ...
            async handleExcelRemove(file) {
                console.log(JSON.stringify(file))
                let name = file.name
                if (this.$isNull(name)) {
                    return;
                }
                if (this.$isNull(this.excelList)) {
                    return;
                }
                const index = this.excelList.findIndex((item) => {
                    return item.name == name
                })
                this.excelList.splice(index, 1);
            },
            async onExcelSuccess(response) {
                console.log('导入Excel成功:' + JSON.stringify(response))
                this.$refs.excelUploader.clearFiles()
            },
            async handle${className}Change(value) {
                this.searchData.${mainId} = value
            },
            async queryAll${className}() {
                ...
                try {
                    Api.queryAll${className}({}).then((res) => {
                        if (res.success) {
