<template>
  <div class="page-view">
      ...
      <el-form :inline="true" ref="pageForm">
        <div class="search-param-view">
          <div class="search-view">
            <!--<div class="search-item-view">
              <el-form-item label="首页轮播图">
                <el-select
                        v-model="searchData.mainSwiperId"
                        :clearable="true"
                        @change="handleMainSwiperChange"
                        placeholder="请选择首页轮播图">
                  <el-option
                          v-for="(item,index) in mainSwiperOptions"
                          :key="item.value"
                          :label="item.text"
                          :value="item.value">
                  </el-option>
          ...
          <div class="search-view">
            ...
            <div class="search-item-view" v-if="isAuth('mainSwiper:add')">
              <el-form-item>
                <el-button
                    size="mini"
                    type="primary"
            ...
            <div class="search-item-view" v-if="isAuth('mainSwiper:export')">
              <el-form-item>
                <el-button
                    size="mini"
                    @click="handleExport"
            ...
            <div class="search-item-view" v-if="isAuth('mainSwiper:batchDelete')">
              <el-form-item>
                <el-button
                    size="mini"
                    type="danger"
    ...
    >
      ...
      >
        <template v-slot="scope">
          <img @click="handlePreView(handleImageUrl(scope.row.mainUrl))"
               :src="handleImageUrl(scope.row.mainUrl)" style="width: 30px;height: 30px;border-radius: 5px;"/>
        </template>
      </el-table-column>
      <el-table-column
          prop="mainTitle"
      ...
      <el-table-column
        ...
        <template v-slot="scope">
          <div class="operation-view">
            <div class="operation-button-view" v-if="isAuth('mainSwiper:view')">
              <el-button
            ...
            <div class="operation-button-view" v-if="isAuth('mainSwiper:edit')">
              <el-button
            ...
            <div class="operation-button-view" v-if="isAuth('mainSwiper:deleteOne')">
              <el-button
...
export default {
  ...
  methods: {
    async handleMainSwiperChange(value) {
      this.searchData.mainSwiperId = value
    },
    async queryAllMainSwiper() {
      try {
        ...
        Api.queryAllMainSwiper({}).then((res) => {
          if (res.success) {
            ...
            this.mainSwiperOptions = new Array();
            data.map((item) => {
              let options = {
                'text': item.mainSwiperName,
                'value': item.mainSwiperId
              }
              this.mainSwiperOptions.push(options)
            })
            console.log('this.mainSwiperOptions:' + JSON.stringify(this.mainSwiperOptions))
            loading.close();
          } else {
            loading.close();
    ...
    deleteOne(data) {
      ...
      mainIdList.push(data.mainSwiperId)
      this.deleteButtonLoading = true
      Api.batchDeleteMainSwiper({
    ...
    batchDelete() {
      ...
      multipleSelection.map(item => {
        mainIdList.push(item.mainSwiperId)
      })
