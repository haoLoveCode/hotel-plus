<template>
  <el-dialog title="编辑系统权限" :center="true" :visible.sync="editVisible">
      ...
      >
        ...
        <div class="update-body-div">
          ...
          <el-form-item label="图标" prop="permissionIcon">
            <el-button
                v-if="$isNull(submitData.permissionIcon)"
                type="text" @click="permIconVisible = true"
                size="mini">
              选择图标
            </el-button>
            <i v-else :class="submitData.permissionIcon"
               @click="permIconVisible = true"
    ...
    <el-dialog
      ...
      <el-tree
          ...
          ref="permissionTree"
          :default-checked-keys="[submitData.parentId]"
          highlight-current
          :check-strictly="true"
...
export default {
  ...
  methods: {
    handleMenuIconCancel() {
      console.log('iconList:' + JSON.stringify(iconList.ICON_LIST))
    ...
    handleParentPermSelect() {
      ...
      if (this.submitData.authPermissionId == selectedItem.authPermissionId) {
        this.$message.error(`不可设置父级菜单为当前菜单`)
        return;
      }
      this.submitData.parentId = selectedItem.authPermissionId
      this.submitData.parentPermissionName = selectedItem.permissionName
      //console.log('选中的菜单:'+JSON.stringify(parentNode));
      this.parentPermVisible = false
    },
    ...
    async initParentPermName() {
      const res = await Api.queryPermissionByMainId({
        mainId: this.submitData.parentId
      });
      if (res.success) {
        let data = res.data;
        if (data) {
          let that = this
          that.submitData.parentPermissionName = data.permissionName
          that.submitData.parentId = data.authPermissionId
        }
      }
    },
    ...
    async showEdit(data) {
      ...
      if (data) {
        ...
        let parentId = this.submitData.parentId
        if (this.$bizConstants.TOP_MENU_NODE == parentId) {
          console.log('顶级节点')
          this.setDefaultParentPerm();
        } else {
          await this.initParentPermName();
        }
      }
      console.log('this.submitData.parentId:' + this.submitData.parentId)
      await this.init();
    ...
    handleSubmit() {
      this.$refs.submitForm.validate((valid) => {
        ...
        try {
          ...
          Api.editAuthPermissionItem(params).then((res) => {
            if (res.success) {
              this.editVisible = false;
