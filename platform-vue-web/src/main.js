...
router.beforeEach(async (to, from, next) => {
    ...
    //console.log('当前页面的token:'+token)
    //如果token不为空的话则跳转到登录界面
    if(!baseUtils.isNull(token)){
        let menuList = store.getters.menuPermissionList;
        if(!menuList || menuList.length == 0){
            console.log('开始重置')
            const loginMeta = await Api.queryLoginMeta({})
            let loginData = loginMeta.data
            //选出所有菜单信息
            //此处重新设置全局的菜单信息
            await store.dispatch("setLoginMeta", loginData).then((resp) => {
                if(baseUtils.isNull(resp)){
                    Message.error('挂载菜单信息错误，返回登录界面');
                    next(bizConstants.LOGIN_PAGE);
                    return;
                }
                //console.log('setLoginMeta:resp:'+JSON.stringify(resp))
                //let menuPermissionList = resp.menuPermissionList
                store.dispatch('setRouter', resp).then(() => {
        ...
        Message.error('登录Token失效，返回登录界面');
        next(bizConstants.LOGIN_PAGE);
        return;
    }
    next();
